...
class MainPage extends React.Component {
  ...
  onRemoveFile = (file) => {
    const newStateFiles = this.state.selectedFiles.filter(f => f.id !== file.id);
    this.setState({ selectedFiles: newStateFiles });
    if (newStateFiles.length < 1) { this.setState({ isSelectedFilesModalOpen: false }); }
  };
  ...
  getExportChoiceList() {
    ...
    if (this.state?.selectedFiles) {
      const collectionChoice = exportChoices.find(choice => choice.value === 'Collection');
      const isSelectionMoreThanOne = this.state.selectedFiles.length > 1;
      collectionChoice.helpText = isSelectionMoreThanOne ? tl('collectionsSelectionInfo') : '';
    ...
    return (
      <div className='appContainer'>
        <PixxioJsdk
          ...
          onSelection={files => {
            if (files?.length > 1) { this.setState({ selectedExport: 'Product' }); }
            this.setState({ selectedFiles: files ? files.reverse() : [], isSelectedFilesModalOpen: true });
          }}
        />
        ...
        <Modal
          ...
          primaryAction={{
            content: tl('continue'),
            onAction: () => this.setState({isSelectedFilesModalOpen: false, isResourcePickerOpen: true })
          }}
          secondaryActions={[{
            content: tl('cancel'),
            onAction: () => this.setState({isSelectedFilesModalOpen: false })
          }]}
        >
          <Modal.Section>
            <ChoiceList
          ...
          <Modal.Section>
            <legend style={{marginBottom: '4px'}}>Selected files</legend>
            {
              this.state.selectedFiles.map(file => {
                return (
                  <div key={file.id} className='flex flex-center border-bottom border-none-last-item' style={{padding: '6px 0'}}>
                    <img src={file.thumbnail} style={{height: 90 + 'px', width: 160 + 'px', objectFit: 'contain'}} />
                    <div className='flex-1 flex flex-column' style={{overflow: 'hidden', margin: '0 12px'}}>
                      <span className='truncate'>{file.file.fileName}</span>
                      <span className='truncate' style={{opacity: '.7', marginTop: 4 + 'px', fontSize: 12 + 'px'}}>{tlByte(file.file.fileSize)}</span>
                    </div>
                    <div className='icon-color hover-icon' onClick={() => this.onRemoveFile(file)}>
